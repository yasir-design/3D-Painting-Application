...
public class ClickPositionManager_Sprint01 : MonoBehaviour
{
    ...
    private float red = 0.8f, green= 0.8f, blue = 0.8f, destroyTime = 3f, timeToDestroy = 3f;
    ...
    void Update () {
        ...
        {
            //checking for a collider in the virtual world that my mousePosition
        ...
        //if check
        if (Input.GetMouseButton(1) && (EventSystem.current.currentSelectedGameObject == null))//right hold and if this statment is true, user is not clicking on the user interface element
        {
            //checking for a collider in the virtual world that my mousePosition
            //is over when the user left clicks or holds
            Ray ray = Camera.main.ScreenPointToRay (Input.mousePosition);
        ...
        if (Input.GetMouseButton(0) && (EventSystem.current.currentSelectedGameObject == null) && Input.mousePosition.x > 200f) //left hold
        {
            ...
            {   
                //NEW
                //float x = Mathf.Clamp(Random.Range(0.5f, 3f) * Mathf.Abs(lastClickPosition.x - clickPosition.x), 0.1f, 5f);
